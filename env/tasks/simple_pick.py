...
# Wrist camera pose in the Franka "hand" link frame. This matches the
# attached-camera example so the eef camera keeps the gripper and target area
# centered while the wrist moves.
...
class SimplePickTask:
    ...
    def _build_scene(self, show_viewer):
        ...
        if USE_NYX:
            ...
            self.eef_cam = self.scene.add_sensor(NyxCameraOptions(
                ...
                near           = 0.02,
                far            = 50.0,
                entity_idx     = self.franka.idx,
                link_idx_local = self.eef.idx_local,
    ...
    def _make_obs_space(self):
        return spaces.Dict({
            ...
            "observation.images.eef": spaces.Box(low=0, high=255, shape=(self.observation_height, self.observation_width, 3), dtype=np.uint8),
        })
